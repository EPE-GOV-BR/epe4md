...
```{r, eval = FALSE}
epe4md_copia_premissas(ano_base = 2021)
```

Essa função criou na raiz do projeto atual as pastas *dados_premissas/2021*, com a copia de todas as planilhas de premissas.
...
```{r, eval = FALSE}
planilha_crescimento <- readxl::read_xlsx("dados_premissas/2021/crescimento_mercado.xlsx")
...
writexl::write_xlsx(planilha_crescimento, "dados/2021/crescimento_mercado.xlsx")
```
...
resultado_crescimento <- epe4md_calcula(
  ...
  premissas_reg = premissas_regulatorias,
  ano_max_resultado = 2027,
  dir_dados_premissas = "dados_premissas/2021"
  )
```
