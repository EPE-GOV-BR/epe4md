...
Em seguida, deve-se definir o argumento `sequencial` que indica se a projeção deve ser realizada de forma sequencial (caso TRUE) ou de forma paralela (caso FALSE).

O próximo parâmetro a ser definido é `filter_uf`, que define se os resultados devem ser filtrados por alguma unidade federativa ou não. Para não filtrar os resultados, defina o parâmetro como "N".
...
A função `epe4md_calcula` possui outros argumentos pré-definidos, que podem ser alterados caso seja de interesse do usuário. A seguir, por exemplo, iremos especificar o horizonte desejado da projeção como sendo 2030. Caso não fosse atribuído valor para esse argumento da função, o resultado iria até 2050. Para mais informações, verificar a documentação da função.
